...
vm.runInNewContext(compiled, { module: cjsModule, exports: cjsModule.exports, fetch: mockFetch, Response });
...
await grantMemberWithToken(session, "2vxsx-fae", "reader");
await revokeMemberWithToken(session, "2vxsx-fae");
...
await finalizeArchiveWithToken(session, [9]);
await beginRestoreWithToken(session, [9], "3");
...
assert.deepEqual(calls[13].body.snapshot_hash, [9]);
