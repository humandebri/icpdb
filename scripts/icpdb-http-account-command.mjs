...
export function buildAccountCommand(command, databaseId, tableNameOrSql, rest, options, auth) {
  ...
  if (command === "operation") {
    return {
      ...
      body: {
        ...
        operation_id: requiredArg(tableNameOrSql, "operation_id")
      },
  ...
  if (command === "create-token") {
    return {
      ...
      body: {
        database_id: requiredArg(databaseId, "database_id"),
        name: requiredArg(tableNameOrSql, "name"),
  ...
  if (command === "revoke-token") {
    return { baseUrl, token, endpoint: "/v1/tokens/revoke", body: { database_id: requiredArg(databaseId, "database_id"), token_id: requiredArg(tableNameOrSql, "token_id") }, ...outputFormatOption(options) };
  }
  ...
  if (command === "grant-member") {
    return {
      ...
      body: {
        database_id: requiredArg(databaseId, "database_id"),
        principal: requiredArg(tableNameOrSql, "principal"),
  ...
  if (command === "revoke-member") {
    return { baseUrl, token, endpoint: "/v1/members/revoke", body: { database_id: requiredArg(databaseId, "database_id"), principal: requiredArg(tableNameOrSql, "principal") }, ...outputFormatOption(options) };
  }
...
function http(baseUrl, token, endpoint, databaseId, options) {
  return { baseUrl, token, endpoint, body: { database_id: requiredArg(databaseId, "database_id") }, ...outputFormatOption(options) };
}
...
function transfer(flag, baseUrl, token, databaseId, filePath, options) {
  return {
    ...
    filePath: requiredArg(filePath, "file"),
    ...outputFormatOption(options)
  };
}
